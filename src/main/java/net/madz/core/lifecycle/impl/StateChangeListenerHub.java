...
public class StateChangeListenerHub {
    ...
    private final ExecutorService executor = Executors
		...
		public Thread newThread(Runnable r) {
		    return new Thread(r, "State Change Notify Thread --"
			    + (++counter));
		}
	    });
	...
	final ArrayList<IStateChangeListener> imageList = new ArrayList<IStateChangeListener>(
		listeners);
	...
	for (final IStateChangeListener listener : imageList) {
		...
		public void run() {
			...
			Logger.getAnonymousLogger().log(
				Level.SEVERE,
				"state change notify failed with listener"
