package net.madz.core.meta.impl;
...
public class MetaDataMap implements Iterable<MetaDataFilterable>, Dumpable {
     ...
     */
    private MetaDataMap(MetaData owner, List<Value> cloneValueList,
	    Map<Object, List<Integer>> valueKeyMap, MetaDataFilter filter,
	    boolean lazyFilter) {
	this.owner = owner;
	this.filter = filter;
	...
	if (null != cloneValueList) {
		...
		Map<Object, List<Integer>> newKeyMap = new HashMap<Object, List<Integer>>(
			(valueKeyMap.size() * 3) / 2);
		this.entryListCache = new ArrayList<MetaDataFilterable>(
			cloneValueList.size());
		for (Value cloneValue : cloneValueList) {
		    if (null != cloneValue) {
			final MetaDataFilterable value = cloneValue
				.getFilteredValue(owner, filter);
			if (null != value) {
			    Value filteredValue = new Value(
				    this.valueList.size(), value,
				    value.getKeySet());
    ...
    public MetaDataMap filter(MetaData newOwner, MetaDataFilter filter,
	    boolean lazyFilter) {
	return new MetaDataMap(newOwner, valueList, valueKeyMap, filter,
		lazyFilter);
    }
	...
	public final boolean equals(Object obj) {
	    return (obj instanceof MetaDataMap.Value)
		    && ((MetaDataMap.Value) obj).position.equals(this.position);
	}
	...
	protected final MetaDataFilterable getFilteredValue(MetaData newOwner,
	...
	public MetaDataFilterable getValue() {
	    Value filteredValue = new Value(position, getFilteredValue(owner,
		    filter), keySet);
	...
	if (null != idList) {
	    ArrayList<MetaDataFilterable> results = new ArrayList<MetaDataFilterable>(
		    idList.size());
	...
	if (null == entryListCache) {
	    entryListCache = new ArrayList<MetaDataFilterable>(
		    this.valueList.size());
	...
	Builder(MetaData owner, int mapSize) {
	    super(owner, null, new HashMap<Object, List<Integer>>(mapSize),
		    MetaDataFilter.NULL_FILTER, false);
	...
	public MetaDataMap filter(MetaData newOwner, MetaDataFilter filter,
		boolean lazyFilter) {
	    Map<Object, List<Integer>> valueKeyMap = new HashMap<Object, List<Integer>>(
		    this.valueKeyMap);
	    for (Map.Entry<Object, List<Integer>> mapEntry : valueKeyMap
		    .entrySet()) {
		mapEntry.setValue(Collections
			.unmodifiableList(new ArrayList<Integer>(mapEntry
				.getValue())));
	    }

	    return new MetaDataMap(newOwner, valueList,
		    Collections.unmodifiableMap(valueKeyMap), filter,
		    lazyFilter);
	}
	...
	public <MD extends MetaDataFilterable> void add(MD metaData) {
	    ...
	    Value valueEntry = new Value(valueList.size(), metaData,
		    metaData.getKeySet());
