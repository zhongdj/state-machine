...
public class StateMetaDataBuilderImpl<R extends IReactiveObject, S extends IState<R, S>>
	extends
	MetaDataBuilderBase<StateMetaData<R, S>, StateMachineMetaData<R, S, ?>>
	implements StateMetaDataBuilder<R, S> {

    private StateMetaDataImpl<R, S> stateMetaDataImpl;

    protected StateMetaDataBuilderImpl(StateMachineMetaData<R, S, ?> parent,
	    String name) {
	super(parent, name);
    }
	...
	try {
	    ...
	    stateMetaDataImpl = new StateMetaDataImpl<R, S>(parent, state,
		    typeEnum, this.path.getName());
