...
public class MetaDataMap implements Iterable<MetaDataFilterable>,  Dumpable {
   ...
   /**
    * Constructor
    * @param owner New MetaData owner for this map.
    * @param cloneValueList
    * @param valueKeyMap
    * @param lazyFilter Enabling will cause this map can be performed any time the keys of the entries do not change.
    *       Passing lazyFilter as false will cause the passed values to be actively filtered, and the key map to
    *       be recreated
    */
   private MetaDataMap( MetaData owner, List<Value> cloneValueList, Map<Object,List<Integer>> valueKeyMap, MetaDataFilter filter, boolean lazyFilter ) {
      ...
      if ( null != cloneValueList ) {
         ...
         //
         // When lazy filtering, copy every Value from the cloneValueList into an UnfilteredValue
...
//      
    ...
    * @param key value's key
    * @return Value implementing the specified class/interface, or null if not found
    * 
    */
   ...
   static class Builder extends MetaDataMap {
      ...
      /**
       * Remove the specified key from the MetaDataMap, any other keys for the same flavor(s) will
       * still exist.
       * @param key Remove specified flavor key
